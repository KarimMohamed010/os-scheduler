...
{
    (void)argc; (void)argv;   /* generator takes no command-line arguments */
    ...
    if (!proc_table) { perror("malloc proc_table"); exit(EXIT_FAILURE); }
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (count == capacity) {
            ...
            if (!proc_table) { perror("realloc proc_table"); exit(EXIT_FAILURE); }
        }
    ...
    if (algo == ALGO_RR) {
        ...
        scanf("%d", &quantum);
        if (quantum < 1) { fprintf(stderr, "Quantum must be >= 1\n"); exit(1); }
        ...
        scanf("%d", &M);
        if (N < 1 || M < 1) { fprintf(stderr, "N and M must be >= 1\n"); exit(1); }
